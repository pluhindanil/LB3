...
def rotate():
    # Get the uploaded file and rotation parameters from the request
    file = request.files.get('file')
    rotate_direction = request.form.get('rotate_direction')
    rotate_value = int(request.form.get('rotate_value'))
    ...
    if rotate_direction == 'left':
        rotate_value *= -1
    rotated_img = img.rotate(rotate_value, expand=True)
